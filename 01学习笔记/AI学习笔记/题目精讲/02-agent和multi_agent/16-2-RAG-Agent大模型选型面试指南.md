...
（意图识别，比如字节会专门用一个微调的小模型做意图识别，意图识别可以视为路由问题，甚至你可以理解为理发店的接待员，它只负责分流，真正给你理发的是Tony老师，Tony老师也有高手和低手区分，主控模型肯定要用旗舰版，子agent可以用弱一点但是微调过的针对某一领域很强的模型）

在生产级 RAG 或 Agent 系统中，入口意图识别通常不会使用旗舰模型，而是采用轻量模型或者专用分类模型，因为 Intent Recognition 本质是分类和路由任务，不需要复杂推理。常见方案是小模型负责快速分类，大模型负责复杂理解和规划；对于低置信度或者复杂请求，再 fallback 到旗舰模型。这种分层路由可以降低成本和延迟，同时保持复杂任务的准确率
