...
划分方式很多 下面这种四层我觉得还行 然后基于deepagent的记忆系统去讲
...
记忆机制分四层：
感知记忆（当前输入的原始内容）
短期记忆（contextwindow里的对话历史 messages列表 摘要+最近N轮）
长期记忆（存在外部数据库、语义检索召回）
实体记忆（结构化提取的关键事实 ） （类似于deepagent的/memory，用户偏好）

CodeBuddy有专门的项目级.memory
Trae的云端experience list
(各个agent都有类似的)

实际设计时要解决三个核心问题：存什么、怎么存、什么时候取出来用，根据信息类型选合适的存储方式，
