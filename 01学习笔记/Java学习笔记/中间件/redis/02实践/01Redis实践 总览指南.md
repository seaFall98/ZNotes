这是**第一篇 Redis 总览篇**。定位是系列开篇：**建立 Redis 的工程认知地图，不急着堆代码**。后续再进入商品缓存、分布式锁、秒杀库存、延迟队列、Stream 等专题案例。文末汇总了案例链接
...
- Spring Boot + RedisTemplate / Redisson 的代码结构设计。
