...
### 1. 带初始值的归约：`T reduce(T identity, BinaryOperator<T> accumulator)`
...
### 2. 无初始值的归约：`Optional<T> reduce(BinaryOperator<T> accumulator)`
...
### 3. 并行流专用归约：`<U> U reduce(U identity, BiFunction<U, ? super T, U> accumulator, BinaryOperator<U> combiner)`
