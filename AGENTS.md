...
- Find notes by filename: use Codex `Glob` with patterns like `**/*关键词*.md`
- Search note content: use Codex `Grep` over `*.md` files
...
- `01学习笔记/` is the main note collection.
  - `AI学习笔记/` covers AI concepts, AI usage practice, Codex/Codex notes, and AI pitfalls.
...
- `.gitignore` excludes `.obsidian/`, `.Codex/`, and `.claudian/`, but those directories may still exist locally. Do not rely on them being committed unless `git status` confirms it.
